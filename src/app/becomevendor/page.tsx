    ...
    const [ formData, setFormData] = useState({
      ...
      const handleSubmit = async (e: any) => {
        ...
        if(formData.password === confirmPassword){
          const submitData = await fetch('/api/registervendor', 
          {method: 'POST',
  ...
  return (
    <div className='w-screen h-screen flex justify-center items-center bg-linear-to-tl from-[var(--third)] to-[var(--text_color)] '>
      ...
      <div className='w-[60%] h-[70%] flex bg-white rounded-xl shadow-[0px_0px_20px_5px_gray] '>
        <div className='w-[50%] h-full flex-col text-center flex justify-center items-center gap-5 border-r-1 border-gray-400'>
          ...
          <p className='text-xl text-[var(--text_color)] font-semibold'>BECOME A SUPPLIER TODAY AND START EXPANDING YOUR BUSINESS DIGITALLY</p>
        </div>
