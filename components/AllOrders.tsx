...
const AllOrders = (props: propsType) => {
  return (
    <div
      className="w-full max-w-4xl p-5 gap-6 items-center border border-gray-200 rounded-lg bg-white shadow-sm transition-transform duration-300 hover:shadow-lg hover:scale-[1.02] flex flex-col md:flex-row"
    >
      <Image
        ...
        <div className="mt-2">
          <span
            className={`inline-block px-4 py-1 rounded-full text-sm font-semibold tracking-wide ${
              props.orderDetail.status === 'Active'
                ? 'bg-yellow-100 text-yellow-800 animate-pulse'
                : props.orderDetail.status === 'Received'
